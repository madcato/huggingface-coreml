...
final class BlenderbotTokenizerTests: XCTestCase {
    var sut: BlenderbotSmallModel!

    override func setUp() async throws {
        sut = try BlenderbotSmallModel()
    }
...
//        do {
//            let sut = try BlenderbotTokenizer.from_pretrained("facebook/blenderbot-small-90M")
//            XCTAssertNotNil(sut)
//        } catch {
//            XCTAssertTrue(false)
//        }
...
//    }
    ...
    func testModelPredict() {
        do {
            let result = try sut.predict(" Hello world")
//            let sut = try BlenderbotTokenizer.from_pretrained("facebook/blenderbot-small-90M")
...
//
//            let input = blenderbot_small_90M_decoderInput(input_ids: MLMultiArray.from(tokens, dims: 2), attention_mask: MLMultiArray.from(masks, dims: 2))
//
//            let model = try blenderbot_small_90M_decoder()
//
//            let output: blenderbot_small_90M_decoderOutput = try model.prediction(input: input)
